...
//server.js configuration related to fetching posts
...
app.post('/api/posts', async(req, res) => {
    try {
        ...
        const newpost = await pool.query(
            "INSERT INTO posttable(title, body, urllink, date) values ($1, $2, $3, $4)    RETURNING*", [post.title, post.body, post.urllink, post.date]
...
app.put('/api/posts/:id', async(req, res) => {
    try {
        ...
        const updatepost = await pool.query( //don't know if date needs to be updated when post has been modified
            "UPDATE posttable SET (title, body, urllink, date) = ($2, $3, $4, $5) WHERE id = $1", [id, post.title, post.body, post.urllink, post.date]
        );
        res.json(updatepost);
    } catch (err) {
        console.error(err.message);
